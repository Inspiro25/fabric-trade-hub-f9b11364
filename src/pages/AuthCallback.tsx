...
const AuthCallback = () => {
  ...
  if (error) {
    return (
      <div className={`min-h-screen flex flex-col items-center justify-center p-4 ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-blue-50'}`}>
        <div className={`p-8 rounded-lg shadow-md max-w-md w-full text-center ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
          ...
          <button
            ...
            className={`px-4 py-2 rounded text-white ${isDarkMode ? 'bg-blue-600 hover:bg-blue-700' : 'bg-blue-500 hover:bg-blue-600'}`}
          >
            Back to Login
          </button>
        </div>
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center ${isDarkMode ? 'bg-gray-900' : 'bg-blue-50'}`}>
      <div className={`animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 ${isDarkMode ? 'border-blue-500' : 'border-blue-500'}`} />
    </div>
  );
};
