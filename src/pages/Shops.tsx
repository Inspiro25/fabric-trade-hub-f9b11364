  ...
  return <div className="min-h-screen bg-gray-50 pb-6">
      ...
      <div className="container mx-auto px-3 py-4">
        {isLoading ? <div className="flex justify-center items-center py-8">
          ...
          </div> : filteredShops.length === 0 ? <div className="text-center py-8 bg-white rounded-lg shadow-sm">
            ...
            {filteredShops.map(shop => <Link to={`/shop/${shop.id}`} key={shop.id} className="block transform transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
                <Card className="overflow-hidden h-full border-none shadow-sm">
                  <div className="relative h-24 bg-gradient-to-r from-kutuku-light to-kutuku-light/50">
                    <img src={shop.coverImage} alt={shop.name} className="w-full h-full object-cover mix-blend-overlay" />
                    {shop.isVerified && <Badge className="absolute top-2 right-2 bg-green-500 text-white border-none text-xs px-1.5 font-normal flex gap-0.5 items-center">
                  ...
                  <CardContent className="relative p-3">
                    <div className="flex items-start mb-2">
                      <div className="h-10 w-10 rounded-full border-2 border-white bg-white shadow-sm -mt-8 overflow-hidden">
                        <img src={shop.logo} alt={shop.name} className="w-full h-full object-cover" onError={e => {
                    ...
                    <Button variant="ghost" size="sm" className="mt-2 w-full text-xs py-1 text-kutuku-primary hover:text-kutuku-primary hover:bg-kutuku-light">
                      View Shop
                    </Button>
                  </CardContent>
                </Card>
