                  ...
                  <ul className="divide-y divide-border">
                    {cartItems.map((item, index) => <li key={`${item.id}-${item.size}-${item.color}`} className="flex flex-col sm:flex-row sm:items-center gap-4 py-0 bg-gray-50">
                        ...
                        <div className="flex-grow bg-slate-50">
                          ...
                          <div className="mt-1">
                            {item.product.salePrice ? <div className="flex items-center gap-2">
                                <span className="font-medium">${item.product.salePrice.toFixed(2)}</span>
                                <span className="text-sm text-muted-foreground line-through">
                                  ${item.product.price.toFixed(2)}
                                </span>
                              </div> : <span className="font-medium">${item.product.price.toFixed(2)}</span>}
                          </div>
                        </div>
                        
                        <div className="flex items-center ml-auto">
                          <div className="flex items-center border rounded-md mr-4">
                            <button type="button" className="w-8 h-8 flex items-center justify-center text-muted-foreground hover:text-foreground" onClick={() => updateQuantity(item.id, item.quantity - 1)} disabled={item.quantity <= 1}>
                              <Minus className="w-3 h-3" />
                              <span className="sr-only">Decrease quantity</span>
                            </button>
                            <span className="w-8 text-center">{item.quantity}</span>
                            <button type="button" className="w-8 h-8 flex items-center justify-center text-muted-foreground hover:text-foreground" onClick={() => updateQuantity(item.id, item.quantity + 1)}>
                          ...
                          <button type="button" className="w-10 h-10 flex items-center justify-center text-muted-foreground hover:text-destructive" onClick={() => removeItem(item.id)}>
                            <X className="w-4 h-4" />
              ...
              <div className={`transition-all duration-500 delay-300 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
                <div className="rounded-lg p-6 border border-border bg-amber-500 py-[10px]">
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex">
                      <Input id="promo-code" value={promoCode} onChange={e => setPromoCode(e.target.value)} placeholder="Enter code" className="rounded-r-none" disabled={isPromoApplied} />
                      <Button variant={isPromoApplied ? "secondary" : "default"} className="rounded-l-none" onClick={applyPromoCode} disabled={isPromoApplied || !promoCode}>
                        {isPromoApplied ? 'Applied' : 'Apply'}
                      </Button>
                    </div>
                    {isPromoApplied && <p className="text-sm text-green-600 mt-2">
                        "DISCOUNT10" applied successfully!
                      </p>}
                    {!isPromoApplied && <p className="text-xs text-muted-foreground mt-2">
