...
export function normalizeProductData(data: any): Product {
  return {
    ...
    colors: data.colors || [],
    sizes: data.sizes || [],
    isNew: Boolean(data.isNew || data.is_new || false),
    is_new: Boolean(data.is_new || data.isNew || false),
    isTrending: Boolean(data.isTrending || data.is_trending || false),
    is_trending: Boolean(data.is_trending || data.isTrending || false),
